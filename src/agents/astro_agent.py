...
class CelestialTrackInput(LangChainBaseModel):
    ...
    object_name: str = LangChainField(description="Name of celestial object (e.g., 'Mars', 'Sirius', 'M31')")
    observation_time: Optional[str] = LangChainField(
        default=None,
        description="ISO format datetime (if None, uses current time)"
    )
...
class StarCatalogQueryInput(LangChainBaseModel):
    ...
    catalog: str = LangChainField(
        default="hipparcos",
        description="Catalog name (hipparcos, tycho, gaia)"
    )
...
class OrbitCalculationInput(LangChainBaseModel):
    ...
    time_since_perihelion: Optional[float] = LangChainField(
        default=0.0,
        description="Time since perihelion (days)"
    )
...
class CoordinateTransformInput(LangChainBaseModel):
    ...
    from_frame: str = LangChainField(
        default="icrs",
        description="Source coordinate frame (icrs, galactic, fk5, altaz)"
    )
    ...
    observation_time: Optional[str] = LangChainField(
        default=None,
        description="Observation time (ISO format, for time-dependent frames)"
    )
...
def track_celestial_object_tool(
    ...
    logger.info(
        ...
        extra={
            "object": object_name,
            "observer_location": f"({observer_lat}, {observer_lon})"
        }
    )
...
def astronomical_calculation_tool(
    ...
    """
    logger.info(
        "Performing astronomical calculation via Astro agent",
        extra={"type": calculation_type}
    )
...
def query_star_catalog_tool(
    ...
    logger.info(
        ...
        extra={
            "catalog": catalog,
            "center": f"RA={ra_center}, Dec={dec_center}",
            "radius": radius
        }
    )
...
def analyze_light_curve_tool(
    ...
    analysis_type: str = "period_detection"
) -> Dict[str, Any]:
    """
    ...
    if analysis_type == "period_detection":
        ...
        variance = sum((m - mean_mag) ** 2 for m in magnitudes) / len(magnitudes) if magnitudes else 0
        ...
        return {
            ...
            "variability_classification": "highly_variable" if std_dev > 0.2 else "moderately_variable" if std_dev > 0.05 else "stable",
...
def transform_coordinates_tool(
    ...
    transformations = {
        ("icrs", "galactic"): {
            "l": 180.0,  # Galactic longitude
            "b": -60.0   # Galactic latitude
        },
        ("icrs", "fk5"): {
            "ra": ra + 0.001,  # Minimal precession correction
            "dec": dec + 0.0005
        },
        ("galactic", "icrs"): {
            "ra": ra + 10.0,
