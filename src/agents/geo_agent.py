...
class SpatialAnalysisInput(LangChainBaseModel):
    ...
    geometries: List[Dict[str, Any]] = LangChainField(
        description="List of geometries to analyze"
    )
...
def geocode_tool(
    ...
    """
    logger.info(
        "Geocoding address via Geo agent",
        extra={"address": address, "country": country}
    )
...
def calculate_distance_tool(
    ...
    """
    logger.info(
        "Calculating distance via Geo agent",
        extra={"unit": unit}
    )
...
def spatial_analysis_tool(
    ...
    """
    logger.info(
        "Performing spatial analysis via Geo agent",
