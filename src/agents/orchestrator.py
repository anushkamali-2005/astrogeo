...
class AgentOrchestrator:
    ...
    def __init__(self, enable_parallel: bool = True, max_retries: int = 2, timeout: int = 600):
        """
    ...
    async def execute(
        ...
        logger.info(
            ...
            extra={"task": task[:100], "required_agents": required_agents},
        )
        ...
        try:
            ...
            response = {
                ...
                "execution_mode": (
                    "parallel" if self.enable_parallel and len(subtasks) > 1 else "sequential"
                ),
    ...
    def _select_agents(self, task: str, context: Optional[Dict[str, Any]]) -> List[AgentType]:
        """
        ...
        logger.info(
            "Agents selected for task", extra={"selected_agents": [a.value for a in selected]}
        )

        return selected

    def _assess_complexity(self, task: str, agents: List[AgentType]) -> TaskComplexity:
        """
    ...
    def _decompose_task(
        self, task: str, agents: List[AgentType], context: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_subtask(self, main_task: str, agent_type: AgentType) -> str:
        """
    ...
    def _get_agent_priority(self, agent_type: AgentType, all_agents: List[AgentType]) -> int:
        """
        ...
        priority_map = {AgentType.DATA: 3, AgentType.GEO: 2, AgentType.ML: 2}

        return priority_map.get(agent_type, 1)

    async def _execute_sequential(self, subtasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for subtask in subtasks:
            ...
            try:
                result = await agent.execute(task=subtask["task"], context=subtask["context"])
                results.append({"agent": agent_type.value, "status": "success", "result": result})

            except Exception as e:
                logger.error(f"Agent {agent_type.value} failed", error=e)
                results.append({"agent": agent_type.value, "status": "failed", "error": str(e)})

        return results

    async def _execute_parallel(self, subtasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for subtask in subtasks:
            ...
            task = asyncio.create_task(
                agent.execute(task=subtask["task"], context=subtask["context"])
            )
        ...
        for agent_type, task in tasks:
            try:
                ...
                logger.error(f"Agent {agent_type.value} failed in parallel execution", error=e)
                results.append({"agent": agent_type.value, "status": "failed", "error": str(e)})

        return results

    def _aggregate_results(
        self, agent_results: List[Dict[str, Any]], original_task: str
    ) -> Dict[str, Any]:
        """
        ...
        for result in successful_results:
            ...
            aggregated["insights"].append(
                {"agent": agent_name, "key_findings": f"Agent {agent_name} completed successfully"}
            )
