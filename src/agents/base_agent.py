...
from src.core.exceptions import AgentError, AgentExecutionError, AgentTimeoutError
...
class BaseAgent(ABC):
    ...
    def _initialize_llm(self) -> ChatOpenAI:
        ...
        try:
            ...
            logger.debug(
                f"LLM initialized for agent: {self.name}", extra={"model": self.model_name}
            )

            return llm

        except Exception as e:
            logger.error(f"Failed to initialize LLM for agent: {self.name}", error=e)
            raise AgentError(
                message=f"Failed to initialize LLM for agent {self.name}", details={"error": str(e)}
            )
    ...
    def _create_agent_executor(self) -> AgentExecutor:
        ...
        try:
            ...
            logger.error(f"Failed to create agent executor for: {self.name}", error=e)
            raise AgentError(
    ...
    async def execute(self, task: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            # Prepare input
            agent_input = {"input": task, **(context or {})}

            # Execute with timeout
            result = await asyncio.wait_for(self._execute_task(agent_input), timeout=self.timeout)
            ...
            raise AgentTimeoutError(
                ...
                details={"task": task, "execution_time_seconds": execution_time},
            )
            ...
            logger.error(
                ...
                error=e,
                extra={"agent": self.name, "execution_time_seconds": execution_time},
            )
    ...
    def add_tool(self, tool: BaseTool) -> None:
        ...
        logger.info(
            f"Tool added to agent: {self.name}", extra={"agent": self.name, "tool": tool.name}
        )
