...
from src.core.exceptions import DataValidationError, MLflowError, ModelTrainingError
...
class ModelTrainingService:
    ...
    async def train_model(
        ...
        logger.info(
            ...
            extra={"model_id": model_id, "model_type": model_type, "dataset": dataset_path},
        )
        ...
        try:
            ...
            with mlflow.start_run(experiment_id=self.experiment_id, run_name=model_id):
                ...
                # Log data stats
                mlflow.log_metrics(
                    {"n_samples": len(X), "n_features": X.shape[1], "n_classes": len(np.unique(y))}
                )
                ...
                mlflow.log_metrics(
                    {"cv_mean_accuracy": cv_scores.mean(), "cv_std_accuracy": cv_scores.std()}
                )
                ...
                mlflow.sklearn.log_model(
                    model, "model", registered_model_name=f"astrogeo_{model_type}"
                )
    ...
    async def _load_data(
        self, dataset_path: str, target_column: str
        ...
        try:
            ...
            if dataset_path.endswith(".csv"):
                ...
                raise DataValidationError(
                    message="Unsupported file format", details={"path": dataset_path}
                )
            ...
            if df.empty:
                raise DataValidationError(
                    message="Dataset is empty", details={"path": dataset_path}
                )
            ...
            if any(pct > 50 for pct in missing_pct.values()):
                logger.warning(
                    "High missing value percentage detected", extra={"missing_pct": missing_pct}
                )
            ...
            raise DataValidationError(
                message="Failed to load dataset", details={"path": dataset_path, "error": str(e)}
            )
    ...
    async def load_model(self, model_id: str, version: Optional[str] = None) -> Any:
        ...
        try:
            ...
            logger.info("Model loaded", extra={"model_id": model_id, "version": version})
    ...
    async def get_model_info(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise MLflowError(
                message="Failed to get model info", details={"model_id": model_id, "error": str(e)}
            )
