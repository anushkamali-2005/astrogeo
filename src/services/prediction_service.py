...
from src.core.exceptions import ModelLoadError, ModelNotFoundError, PredictionError
from src.core.logging import get_logger, log_async_execution_time
from src.database.models import MLModel, Prediction
from src.schemas.requests import BatchPredictionRequest, PredictionRequest
from src.schemas.responses import BatchPredictionResponse, PredictionResponse
...
class PredictionService:
    ...
    def __init__(self):
        ...
        logger.info(
            "Prediction service initialized", extra={"redis_enabled": self.redis_cache.enabled}
        )

    async def _get_model_from_db(
        self, db: AsyncSession, model_id: Optional[UUID] = None, model_name: Optional[str] = None
    ) -> MLModel:
        """
    ...
    async def _load_model(self, model: MLModel) -> Any:
        ...
        try:
            ...
            raise ModelLoadError(
                model_name=model.name, details={"error": str(e), "model_path": model.model_path}
            )
        ...
        try:
            ...
            if missing:
                raise PredictionError(
                    message="Missing required features", details={"missing_features": list(missing)}
                )
            ...
            raise PredictionError(message="Invalid features", details={"error": str(e)})

    def _make_prediction(
        self, model: Any, features: Dict[str, Any], return_probabilities: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if return_probabilities:
            result["probabilities"] = {"urban": 0.85, "suburban": 0.12, "rural": 0.03}
    ...
    async def predict(
        ...
        try:
            ...
            model = await self._get_model_from_db(
                db, model_id=request.model_id, model_name=request.model_name
            )

            # Check cache if enabled
            cache_key = None
            if use_cache and self.redis_cache.enabled:
                cache_key = self._generate_cache_key(str(model.id), request.features)
                cached_result = self.redis_cache.get("prediction", cache_key)

                if cached_result:
                    logger.info("Prediction served from cache", extra={"model_id": str(model.id)})
            ...
            result = self._make_prediction(
                loaded_model, request.features, request.return_probabilities
            )
            ...
            if use_cache and cache_key and self.redis_cache.enabled:
                self.redis_cache.set("prediction", cache_key, response, ttl=self.cache_ttl)
                logger.debug(
                    "Prediction cached", extra={"model_id": str(model.id), "ttl": self.cache_ttl}
                )
    ...
    async def batch_predict(
        self, db: AsyncSession, request: BatchPredictionRequest, user_id: Optional[UUID] = None
        ...
        try:
            ...
            for idx, features in enumerate(request.features_list):
                try:
                    # Make prediction
                    result = self._make_prediction(
                        loaded_model, features, request.return_probabilities
                    )
                    ...
                    logger.error(f"Batch prediction failed for index {idx}", error=e)
                    predictions.append({"index": idx, "error": str(e), "status": "failed"})
                    failed += 1
