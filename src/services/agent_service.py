...
class AgentService:
    ...
    def _initialize_agents(cls) -> None:
        ...
        """
        try:
            cls._agent_registry = {"data": DataAgent(), "ml": MLAgent(), "geo": GeoAgent()}
            cls._orchestrator = AgentOrchestrator()

            logger.info(
                "Agent registry initialized", extra={"agent_count": len(cls._agent_registry)}
            )

        except Exception as e:
            logger.error("Failed to initialize agent registry", error=e)
            raise AgentError(
                message="Failed to initialize agent registry", details={"error": str(e)}
            )
    ...
    async def execute_agent(
        ...
        try:
            ...
            logger.error(
                ...
                error=e,
                extra={"agent_type": agent_type, "execution_time_seconds": execution_time},
            )
            ...
            raise AgentExecutionError(
                agent_name=agent_type,
                details={"task": task, "error": str(e), "execution_time_seconds": execution_time},
            )

    async def orchestrate_multi_agent(
        self, task: str, required_agents: Optional[List[str]] = None, user_id: Optional[UUID] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise AgentExecutionError(
                agent_name="orchestrator", details={"task": task, "error": str(e)}
            )
    ...
    async def get_agent_metrics(
        self, agent_name: Optional[str] = None, time_window_hours: int = 24
    ) -> Dict[str, Any]:
        """
        ...
        # Build query
        query = select(AgentExecution).where(AgentExecution.created_at >= start_time)
        ...
        return {
            ...
            "success_rate": (
                round(successful / total_executions * 100, 2) if total_executions > 0 else 0
            ),
    ...
    def get_available_agents(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {name: agent.get_info() for name, agent in self._agent_registry.items()}
