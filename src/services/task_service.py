...
from src.core.exceptions import TaskExecutionError, ResourceNotFoundError
...
def batch_predict_task(
    ...
    try:
        ...
        celery_logger.info(
            ...
            extra={"num_predictions": len(predictions)}
        )
...
def cleanup_old_data_task(self, days_old: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        celery_logger.info(
            "Cleanup completed",
            extra={"deleted_items": result["deleted_predictions"]}
        )
...
class TaskService:
    ...
    def submit_training_task(
        ...
        logger.info(
            ...
            extra={"task_id": task.id, "model_id": model_id}
        )
        return task.id
    ...
    def submit_prediction_task(
        ...
        logger.info(
            "Submitted prediction task",
            extra={"task_id": task.id, "num_samples": len(input_data)}
        )
        return task.id
