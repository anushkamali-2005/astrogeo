...
class EncodingTransformer(DataTransformer):
    ...
    def _fit_impl(self, data: pd.DataFrame) -> None:
        ...
        columns_to_encode = (
            self.columns or data.select_dtypes(include=["object", "category"]).columns
        )
    ...
    def _transform_impl(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.strategy == self.Strategy.LABEL:
            ...
            data = pd.get_dummies(
                data, columns=list(columns_to_encode), prefix=list(columns_to_encode)
            )
...
class TransformationPipeline:
    ...
    def add_normalization(
        ...
        self.transformers.append(NormalizationTransformer(columns, strategy))
        return self
    ...
    def add_encoding(
        ...
        """Add encoding step."""
        self.transformers.append(EncodingTransformer(columns, strategy))
        return self
    ...
    def add_imputation(
        ...
        """Add imputation step."""
        self.transformers.append(ImputationTransformer(strategy, fill_value, columns))
        return self
    ...
    def add_feature_engineering(
        ...
        self.transformers.append(FeatureEngineeringTransformer(feature_functions))
        return self
