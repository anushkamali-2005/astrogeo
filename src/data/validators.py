...
class QualityValidator(DataValidator):
    ...
    async def _validate_impl(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if len(data) == 0:
            result.add_issue(
                level=ValidationLevel.ERROR, message="Dataset is empty", details={"row_count": 0}
            )
...
class GeospatialValidator(DataValidator):
    ...
    async def _validate_impl(self, data: pd.DataFrame) -> ValidationResult:
        ...
        invalid_lat = ((data[self.latitude_column] < -90) | (data[self.latitude_column] > 90)).sum()
        ...
        invalid_lon = (
            (data[self.longitude_column] < -180) | (data[self.longitude_column] > 180)
        ).sum()
...
class ValidationPipeline:
    ...
    def add_schema_validation(
        ...
        self.validators.append(SchemaValidator(required_columns, column_types))
        return self

    def add_quality_validation(
        self, max_missing_percent: float = 20.0, unique_columns: Optional[List[str]] = None
    ) -> "ValidationPipeline":
        """Add quality validation."""
        self.validators.append(QualityValidator(max_missing_percent, unique_columns))
        return self
    ...
    def add_geospatial_validation(
        ...
        self.validators.append(GeospatialValidator(latitude_column, longitude_column))
        return self
