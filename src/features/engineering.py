...
class NumericalScaler(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y=None):
        ...
        logger.info(
            "Numerical scaler fitted", extra={"method": self.method, "n_features": len(cols)}
        )
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y=None):
        ...
        logger.info(
            "Categorical encoder fitted", extra={"method": self.method, "n_features": len(cols)}
        )
    ...
    def transform(self, X: Union[pd.DataFrame, np.ndarray]) -> pd.DataFrame:
        ...
        if self.method == "label":
            ...
            encoded_df = pd.DataFrame(encoded, columns=feature_names, index=X_df.index)
...
class MissingValueImputer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, strategy: str = "mean", fill_value: Any = None, n_neighbors: int = 5):
        """
        Initialize imputer.
...
# Export
__all__ = ["NumericalScaler", "CategoricalEncoder", "MissingValueImputer"]
