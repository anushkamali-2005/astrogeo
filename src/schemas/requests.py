...
class PredictionRequest(BaseModel):
    ...
    model_id: Optional[UUID] = Field(
        None,
        description="Model ID (uses latest production model if not specified)"
    )
    model_name: Optional[str] = Field(
        None,
        description="Model name (alternative to model_id)"
    )
...
class AgentExecutionRequest(BaseModel):
    ...
    # Options
    timeout: conint(ge=10, le=600) = Field(
        300,
        description="Execution timeout in seconds (10-600)"
    )
    save_to_database: bool = Field(
        True,
        description="Save execution history to database"
    )
...
class MultiAgentRequest(BaseModel):
    ...
    agents: Optional[List[AgentType]] = Field(
        None,
        description="Specific agents to use (auto-select if not provided)"
    )
    
    # Options
    max_iterations: conint(ge=1, le=20) = Field(
        10,
        description="Maximum agent iterations"
    )
    timeout: conint(ge=30, le=1800) = Field(
        600,
...
class ModelTrainingRequest(BaseModel):
    ...
    # Model info
    model_name: constr(min_length=1, max_length=255) = Field(
        ...,
    ...
    feature_columns: Optional[List[str]] = Field(
        None,
        description="Feature columns (uses all if not specified)"
    )
