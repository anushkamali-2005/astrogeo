...
async def require_admin(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    if not current_user.get("is_superuser", False):
        raise AuthorizationError(
            message="Admin privileges required", details={"user_id": current_user.get("sub")}
        )
    return current_user
...
async def list_models(
    ...
    Returns:
        dict: List of models
    """
    logger.info("Listing models", extra={"status_filter": status, "admin_id": admin.get("sub")})
...
async def deploy_model(
    ...
    logger.info(
        ...
        extra={"model_id": str(model_id), "old_status": old_status, "new_status": model.status},
    )
...
async def archive_model(
    model_id: UUID, db: AsyncSession = Depends(get_db), admin: Dict = Depends(require_admin)
) -> Dict[str, Any]:
    """
    Archive a model.
    ...
    """
    logger.info("Archiving model", extra={"model_id": str(model_id), "admin_id": admin.get("sub")})
...
async def list_users(
    ...
    Returns:
        dict: List of users
    """
    logger.info("Listing users", extra={"active_filter": is_active, "admin_id": admin.get("sub")})
    ...
    return {"success": True, "data": users_data, "message": f"Retrieved {len(users)} users"}
...
async def toggle_user_active(
    ...
    logger.info(
        ...
        extra={"user_id": str(user_id), "is_active": is_active, "admin_id": admin.get("sub")},
    )
...
async def get_system_stats(
    ...
    # Count predictions
    predictions_count = await db.execute(select(func.count()).select_from(Prediction))
    total_predictions = predictions_count.scalar()

    # Count models
    models_count = await db.execute(select(func.count()).select_from(MLModel))
    ...
    return {"success": True, "data": stats, "message": "System statistics retrieved successfully"}
...
async def detailed_health_check(
    ...
    if health_request.include_cache:
        ...
        components["cache"] = ComponentHealth(
            status="healthy", response_time_ms=1.2, details={"type": "Redis"}
        )
    ...
    if health_request.include_mlflow:
        ...
        components["mlflow"] = ComponentHealth(
            status="healthy", response_time_ms=23.4, details={"tracking_uri": "configured"}
        )
    ...
    return {"success": True, "data": health_response, "message": "Health check completed"}
...
async def cleanup_old_predictions(
    days: int = 90, db: AsyncSession = Depends(get_db), admin: Dict = Depends(require_admin)
) -> Dict[str, Any]:
    """
    ...
    """
    logger.info("Cleaning up old predictions", extra={"days": days, "admin_id": admin.get("sub")})
    ...
    result = await db.execute(select(Prediction).where(Prediction.created_at < cutoff_date))
...
async def get_system_config(admin: Dict = Depends(require_admin)) -> Dict[str, Any]:
    """
    ...
    return {"success": True, "data": config, "message": "Configuration retrieved successfully"}
