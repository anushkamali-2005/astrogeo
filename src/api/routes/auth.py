...
from src.core.security import JWTManager, hash_password, rate_limiter, verify_password
...
async def get_current_user_from_token(
    ...
    try:
        ...
        raise AuthenticationError(
            message="Invalid authentication credentials", details={"error": str(e)}
        )
...
def check_rate_limit(request: Request, max_requests: int, window_seconds: int) -> None:
    ...
    if not rate_limiter.is_allowed(client_ip, max_requests, window_seconds):
        ...
        raise RateLimitExceededError(
            limit=max_requests, window=f"{window_seconds}s", details={"remaining": remaining}
        )
...
async def register(
    data: UserRegisterRequest, request: Request, db: AsyncSession = Depends(get_db)
) -> UserProfileResponse:
    """
    ...
    if existing_email:
        raise DuplicateRecordError(resource="User", details={"field": "email", "value": data.email})

    # Check if username exists
    existing_username = await user_repo.get_by_username(data.username)
    if existing_username:
        raise DuplicateRecordError(
            resource="User", details={"field": "username", "value": data.username}
        )
    ...
    logger.info(
        "User registered successfully", extra={"user_id": str(user.id), "email": user.email}
    )
...
async def login(
    data: UserLoginRequest, request: Request, db: AsyncSession = Depends(get_db)
) -> TokenResponse:
    """
...
async def logout(current_user: Dict[str, Any] = Depends(get_current_user_from_token)) -> None:
    """
...
async def update_current_user_profile(
    ...
    if data.email is not None:
        ...
        if existing and str(existing.id) != current_user["id"]:
            raise DuplicateRecordError(
                resource="User", details={"field": "email", "value": data.email}
            )
...
async def reset_password(data: ResetPasswordRequest, db: AsyncSession = Depends(get_db)) -> None:
    ...
    try:
        ...
        raise AuthenticationError(
            message="Invalid or expired reset token", details={"error": str(e)}
        )
