...
from src.schemas.responses import BatchPredictionResponse, PredictionResponse, SuccessResponse
...
async def predict(
    ...
    try:
        ...
        result = await prediction_service.predict(
            db=db, request=request, user_id=UUID(user_id) if user_id else None
        )

        return SuccessResponse(data=result, message="Prediction completed successfully")
...
async def batch_predict(
    ...
    try:
        ...
        if len(request.features_list) > 1000:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Batch size exceeds maximum of 1000"
            )
...
async def submit_feedback(
    ...
    try:
        ...
        logger.info(
            "Feedback submitted",
            extra={"prediction_id": str(prediction_id), "score": feedback_score},
        )
