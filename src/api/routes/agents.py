...
from src.schemas.responses import AgentExecutionResponse, PaginatedResponse, SuccessResponse
...
def get_agent_by_type(agent_type: str):
    ...
    Returns:
        Agent instance
    """
    agents = {"data": data_agent, "ml": ml_agent, "geo": geo_agent}
...
async def execute_agent(
    ...
    if not agent:
        raise AgentExecutionError(
            agent_name=request.agent_type, details={"reason": "Unknown agent type"}
        )
    ...
    try:
        ...
        logger.error(f"Agent execution failed", error=e, extra={"agent_type": request.agent_type})
...
async def orchestrate_agents(
    ...
    try:
        ...
        result = await orchestrator.execute(task=request.task, required_agents=required_agents)

        # Save orchestration to database
        user_id = UUID(current_user.get("sub")) if current_user.get("sub") else None
        execution = await save_agent_execution(
            db=db, agent_name="orchestrator", task=request.task, result=result, user_id=user_id
        )
        ...
        logger.error("Multi-agent orchestration failed", error=e)
        raise AgentExecutionError(agent_name="orchestrator", details={"error": str(e)})
...
async def get_agent_history(
    ...
    logger.info(
        ...
        extra={"agent_name": agent_name, "status": status, "user_id": current_user.get("sub")},
    )
    ...
    # Get total count
    count_query = (
        select(func.count()).select_from(AgentExecution).where(AgentExecution.user_id == user_id)
    )
    if agent_name:
...
async def get_execution_details(
    ...
    """
    logger.info("Retrieving execution details", extra={"execution_id": str(execution_id)})
    ...
    if execution.user_id != user_id:
        ...
        raise AuthorizationError(message="Not authorized to view this execution")
...
async def get_agents_info(current_user: Dict = Depends(get_current_user)) -> Dict[str, Any]:
    """
    ...
    return {"success": True, "data": agents_info, "message": "Retrieved information for 3 agents"}
...
async def get_agent_info(
    ...
    if not agent:
        raise AgentExecutionError(agent_name=agent_type, details={"reason": "Unknown agent type"})
