...
class TestPredictionServiceCaching:
    ...
    @pytest.mark.asyncio
    async def test_prediction_without_cache(self, prediction_service, mock_db, sample_request):
        """Test prediction without using cache."""
        with (
            patch.object(prediction_service, "_get_model_from_db") as mock_get_model,
            ...
            result = await prediction_service.predict(
                db=mock_db, request=sample_request, use_cache=False
            )
    ...
    async def test_prediction_cache_miss_then_hit(
        self, prediction_service, mock_db, sample_request
    ):
        ...
        with (
            ...
            mock_predict.return_value = {"prediction": "urban", "confidence": 0.85}

            # First call - cache miss
            result1 = await prediction_service.predict(
                db=mock_db, request=sample_request, use_cache=True
            )
