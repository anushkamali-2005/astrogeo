...
class TestPredictionEndpoints:
    ...
    async def test_make_prediction(self, authenticated_client: tuple, test_model, sample_features):
    ...
    async def test_make_prediction_by_name(
        self, authenticated_client: tuple, test_model, sample_features
    ):
    ...
    async def test_batch_prediction(self, authenticated_client: tuple, test_model, sample_features):
    ...
    async def test_prediction_without_auth(self, client: AsyncClient, sample_features):
        """Test prediction without authentication."""
        response = await client.post("/api/v1/predict", json={"features": sample_features})

        assert response.status_code == 401  # Unauthorized

    async def test_batch_size_limit(self, authenticated_client: tuple, test_model):
        ...
        response = await client.post(
            "/api/v1/predict/batch",
            json={"model_id": str(test_model.id), "features_list": large_batch},
            headers=auth_headers,
        )
    ...
    async def test_get_prediction(
        self, authenticated_client: tuple, test_model, sample_features, db_session
    ):
        ...
        # Get prediction
        response = await client.get(f"/api/v1/predictions/{prediction.id}", headers=auth_headers)
    ...
    async def test_submit_feedback(
        ...
        response = await client.post(
            ...
            params={"feedback_score": 5, "feedback_comment": "Great prediction!"},
            headers=auth_headers,
        )
...
class TestErrorHandling:
    ...
    async def test_invalid_model_id(self, authenticated_client: tuple, sample_features):
        """Test prediction with invalid model ID."""
        client, auth_headers = authenticated_client

        response = await client.post(
            "/api/v1/predict",
            json={"model_id": "00000000-0000-0000-0000-000000000000", "features": sample_features},
            headers=auth_headers,
        )

        assert response.status_code == 404  # Not found

    async def test_missing_features(self, authenticated_client: tuple, test_model):
        """Test prediction with missing features."""
        client, auth_headers = authenticated_client

        response = await client.post(
            "/api/v1/predict",
            json={"model_id": str(test_model.id), "features": {}},  # Empty features
            headers=auth_headers,
        )
