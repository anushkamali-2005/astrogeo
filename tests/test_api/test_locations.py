...
async def authenticated_client(client):
    ...
    login_response = await client.post(
        "/auth/login", json={"email": "location@example.com", "password": "SecurePass123!"}
    )
...
class TestLocationCreation:
    ...
    async def test_create_location_unauthorized(self, client):
        ...
        location_data = {"name": "Test", "latitude": 34.0, "longitude": -118.0}
...
class TestLocationRetrieval:
    ...
    async def test_get_location_by_id(self, authenticated_client):
        ...
        create_response = await authenticated_client.post(
            "/locations", json={"name": "GetTest", "latitude": 40.7128, "longitude": -74.0060}
        )
    ...
    async def test_list_locations(self, authenticated_client):
        ...
        await authenticated_client.post(
            "/locations", json={"name": "Location 1", "latitude": 34.0, "longitude": -118.0}
        )

        await authenticated_client.post(
            "/locations", json={"name": "Location 2", "latitude": 35.0, "longitude": -119.0}
        )
...
class TestLocationUpdate:
    ...
    async def test_update_location(self, authenticated_client):
        ...
        # Create location
        create_response = await authenticated_client.post(
            "/locations", json={"name": "UpdateTest", "latitude": 40.0, "longitude": -74.0}
        )
        location_id = create_response.json()["id"]

        # Update location
        update_data = {"name": "Updated Name", "description": "New description"}
        response = await authenticated_client.put(f"/locations/{location_id}", json=update_data)
...
class TestLocationDeletion:
    ...
    async def test_delete_location(self, authenticated_client):
        ...
        # Create location
        create_response = await authenticated_client.post(
            "/locations", json={"name": "DeleteTest", "latitude": 40.0, "longitude": -74.0}
        )
        location_id = create_response.json()["id"]
...
class TestSpatialQueries:
    ...
    async def test_find_nearby_locations(self, authenticated_client):
        ...
        # Create locations
        await authenticated_client.post(
            "/locations", json={"name": "Nearby 1", "latitude": 34.0522, "longitude": -118.2437}
        )

        await authenticated_client.post(
            "/locations", json={"name": "Nearby 2", "latitude": 34.0600, "longitude": -118.2500}
        )

        # Search nearby
        response = await authenticated_client.get(
            "/locations/nearby",
            params={"latitude": 34.0522, "longitude": -118.2437, "radius_km": 10},
        )
