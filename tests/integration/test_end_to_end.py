...
class TestUserWorkflow:
    ...
    async def test_user_registration_and_authentication(self, client):
        ...
        # Step 2: Login
        login_data = {"email": "workflow@example.com", "password": "WorkflowPass123!"}
...
class TestLocationWorkflow:
    ...
    async def test_location_crud_workflow(self, client):
        ...
        login_response = await client.post(
            "/auth/login", json={"email": "location@workflow.com", "password": "Pass123!"}
        )
        ...
        create_response = await client.post("/locations", json=create_data, headers=headers)
        ...
        get_response = await client.get(f"/locations/{location_id}", headers=headers)
        ...
        update_response = await client.put(
            f"/locations/{location_id}", json=update_data, headers=headers
        )
        ...
        delete_response = await client.delete(f"/locations/{location_id}", headers=headers)
...
class TestMLWorkflow:
    ...
    async def test_model_prediction_workflow(self, client):
        ...
        login_response = await client.post(
            "/auth/login", json={"email": "ml@workflow.com", "password": "MLPass123!"}
        )
...
class TestAgentWorkflow:
    ...
    async def test_agent_execution_workflow(self, client):
        ...
        login_response = await client.post(
            "/auth/login", json={"email": "agent@workflow.com", "password": "AgentPass123!"}
        )
