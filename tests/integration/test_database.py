...
from src.database.models import AgentExecution, Dataset, MLModel, Prediction, User
from src.database.repositories import MLModelRepository, PredictionRepository, UserRepository
...
class TestDatabaseTransactions:
    ...
    async def test_update_model_status(self, db_session):
        ...
        # Update status
        updated = await repo.update(model.id, {"status": "production"})
...
class TestDatabaseConstraints:
    ...
    async def test_unique_username_constraint(self, db_session):
        ...
        # Create first user
        await repo.create(
            {"username": username, "email": "unique1@example.com", "hashed_password": "hashed"}
        )

        # Try to create duplicate
        with pytest.raises(Exception):  # Will raise IntegrityError
            await repo.create(
                {"username": username, "email": "unique2@example.com", "hashed_password": "hashed"}
            )
